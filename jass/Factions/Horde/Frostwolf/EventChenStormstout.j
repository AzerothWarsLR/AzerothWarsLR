...
library EventChenStormstout initializer OnInit requires Persons, WarsongConfig, FrostwolfConfig, MannorothConfig, NewHordeConfig
  ...
  private function PersonGrantChen takes Person whichPerson returns nothing
    call DisplayTextToForce(whichPerson.team.players, "Chen Stormstout has joined the " + whichPerson.faction.prefixCol + whichPerson.faction.name + "|r.")
    call SetPlayerTechResearched(ChenOwner.p, CHEN_RESEARCH, 0)
  ...
  private function PersonFactionChanges takes nothing returns nothing
    ...
    local integer i = 0
    local Person tempPerson
    if ChenOwner == triggerPerson and PrimaryEligibleFactions.contains(prevFaction) and not PrimaryEligibleFactions.contains(triggerPerson.faction) then
      loop
        exitwhen i == SecondaryEligibleFactions.size
        set tempPerson = PersonsByFaction[SecondaryEligibleFactions[i]]
  ...
  private function OnInit takes nothing returns nothing
    ...
    call PrimaryEligibleFactions.add(FACTION_MANNOROTH)  
